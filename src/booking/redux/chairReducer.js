...
export const chairReducer = (state = initialState, action) => {
  switch (action.type) {
    case "CHANGE_CHAIR": {
      const newChair = [...state.chairSelectedList];
      const foundChair = newChair.findIndex(
        (item) => item.soGhe === action.payload.soGhe
      );

      if (foundChair !== -1) {
        newChair.splice(foundChair, 1);
      } else {
        newChair.push(action.payload);
      }

      return { ...state, chairSelectedList: newChair };
    }

    case "PAY": {
      const data = state.chairList.map((item) => {
        const dsGhe = item.danhSachGhe.map((chair) => {
          state.chairSelectedList.forEach((ele) => {
            if (chair.soGhe === ele.soGhe) {
              chair.daDat = true;
            }
          });
          return chair;
